...
if(isset($_POST['update_profile'])){
   ...
   mysqli_query($con, "UPDATE `user` SET firstName = '$update_name', email = '$update_email' WHERE userId = '$userId'") or die('query failed');
   ...
   if(!empty($update_pass) || !empty($new_pass) || !empty($confirm_pass)){
      if($update_pass != $old_pass){
         ...
         mysqli_query($con, "UPDATE `users` SET password = '$confirm_pass' WHERE id = '$userId'") or die('query failed');
   ...
   if(!empty($update_image)){
      if($update_image_size > 2000000){
         ...
         $image_update_query = mysqli_query($con, "UPDATE `user` SET Image = '$update_image' WHERE userId = '$userId'") or die('query failed');
...
<div class="update-profile">
   ...
   <form action="avatar_updateProf.php" method="post" enctype="multipart/form-data">
      <?php
         if($fetch['image'] == ''){
